...
try{
    let config = JSON.parse(fs.readFileSync('snerrfile.json')).config
    _.each(config, (con) => {
...
module.exports = {
    err : (snerr, err, addMessage) => {
        ...
        const error = {code: snerr,err: err}
        ...
        try{
            return fs.writeFileSync('snerrfile.json', JSON.stringify(file, null, 2))
        }
        catch(e){
